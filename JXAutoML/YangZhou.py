...
class YangZhou:
    ...
    def _get_max_surrounding_mean_sd(self):
        ...
        best_combo_surrounding_combos = self._get_surrounding_combos(
            self.best_combo, self._surrounding_vectors)
    ...
    def _find_horizontal(self, surrounding_combos, core):
        ...
        for i in range(len(core)):
            ...
            for move in [-1, 1]:
                ...
                direction.append(
                    [move if j == i else 0 for j in range(len(core))])
    ...
    def _find_diagonal(self, core, indices):
        ...
        for diagonal in diagonals:
            treatment.append(self._get_diagonal_treatment(
                core, diagonal, indices))
    ...
    def _get_diagonal_treatment(self, core, diagonal, indices):
        ...
        treatment = self._get_surrounding_combos(
            core, self._get_diag_treatment_vectors(indices, diagonal))
    ...
    def _get_blocks(self, core, surrounding_combos, indices):
        ...
        hor_treatment, hor_null, hor_dir = self._find_horizontal(
            surrounding_combos, core)

        diag_treatment, diag_null, vert_dir = self._find_diagonal(
            core, indices)
        ...
        for i in range(len(treatment)):
            ...
            if bool_inc:  # check if the treatment mean > null mean
                ...
                p_val = stats.ttest_ind(list(self._get_treat_or_null_tune_scores(treatment[i]).values()),
                                        list(self._get_treat_or_null_tune_scores(
                                            null[i]).values()),
                ...
                if p_val < 0.05:  # check if p value < 0.05
                    ...
                    proposed_new_core = tuple(
                        [direction[i][j] + core[j] for j in range(len(core))])

                    # get the max combo of the treatment
                    max_combo_of_treatment = self._dict_arg_max(
                        self._get_treat_or_null_tune_scores(treatment[i]))
                    ...
                    # if not, put in the max core
                        ...
                        if not self.checked_core[tuple(max_combo_of_treatment)]:
                            tmp_new_cores.append(
                                [tuple(max_combo_of_treatment), p_val])
    ...
    def _get_new_cores(self, core):
        ...
        surrounding_combos = self._get_surrounding_combos(
            core, self._surrounding_vectors)

        indices = self.get_indices(core)

        # put combos into treatments and nulls
        treatment, null, direction = self._get_blocks(
            core, surrounding_combos, indices)
    ...
    def tune(self, key_stats_only=False):
        ...
        self._restarts = 0
        # Initial Round of Guidance, starting at the core
        self._GuidanceSystem(self._core)
        self._restarts += 1
