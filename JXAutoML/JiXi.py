...
class JiXi:
    ...
    def change_tuning_style(self, type, seed=None, outer_most_layer=2, randomise=True):
        ...
        if type == 'a':  # Sorted order (nested loops)
            ...
            if randomise:
                print(
                    f'Changed tuning order to "Layer by Layer": {self.outmost_layer} Layers, randomised')
            else:
                print(
                    f'Changed tuning order to "Layer by Layer": {self.outmost_layer} Layers, not randomised')

        elif type == 'd':  # Diagonal + Horizontal first, before conducting layer by layer
            self._change_tuning_style_d()
            print(
                f'Changed tuning order to Diag-Hor -> "Layer by Layer": {self.outmost_layer} Layers, randomised')
    ...
    def _get_relative_combos(self):
        ...
        self._relative_combos = [[combo[j] - self._core[j]
                                  for j in range(len(self.n_items))] for combo in self.combos]
    ...
    def _get_both_combos(self):
        ...
        self._both_combos = [[self.combos[i], self._relative_combos[i]]
    ...
    def _change_tuning_style_d(self):
        ...
        self._change_tuning_style_c(outmost_layer=max(
            self.n_items), randomise=True, seed=self._seed)
    ...
    def _get_abs_max(self):
        ...
        max_pos = max([self.n_items[i] - self._core[i]
                      for i in range(len(self.n_items))])
    ...
    def _get_diag_combos(self):
        ...
        for i in range(self._abs_max):
            diag_rel_combos.extend([[(i+1)*pos_neg_combo[j] for j in range(len(self.n_items))]
                                   for pos_neg_combo in self._pos_neg_combos])

        tmp_diag_combos = [[combo[j] + self._core[j]
                            for j in range(len(self.n_items))] for combo in diag_rel_combos]
        ...
        for combo in tmp_diag_combos:
            ...
            for i in range(len(combo)):
                # if outside field space then eliminate
                if combo[i] < 0 or combo[i] >= self.n_items[i]:
                    trigger = 0
                    break
